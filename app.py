...
def load_data(sheet_id):
    try:
        ...
        # Clean strings to prevent filter mismatches
        df['Location'] = df['Location'].astype(str).str.strip()
        df = df[df['Location'] != ""]
        
        if 'Date' in df.columns:
            # FIXED: dayfirst=True for SG/MY date formats
            df['Date_dt'] = pd.to_datetime(df['Date'], format='mixed', dayfirst=True, errors='coerce')
...
st.markdown("""
    ...
    .table-header { font-size: 28px; font-weight: bold; padding: 10px; border-radius: 5px; margin-bottom: 10px; text-align: center; }
...
with c2:
    try:
        ...
        u_locs = sorted(pre_load['Location'].unique())
    except:
        u_locs = []
...
if st.session_state.search_clicked:
    try:
        current_id = SHEET_MAPPING[selected_store]
        raw_data = load_data(current_id)
        ...
        if not df_display.empty:
            ...
            # SAFE FILTERS
            m_date = (df_display['Date'] >= start) & (df_display['Date'] <= end)
            m_loc = True if "ALL" in sel_locs else df_display['Location'].isin(sel_locs)
            m_type = df_display['Type'].isin(sel_types)
            
            if "ALL" in sel_origins:
                m_orig = True
            else:
                m_orig = df_display['Origin'].apply(lambda x: any(o in str(x) for o in sel_origins))

            filtered_df = df_display[m_date & m_loc & m_type & m_orig].copy()

            if not filtered_df.empty:
                st.divider()
                st.markdown(f"### Results for {selected_store} ({start} to {end})")
                tab1, tab2 = st.tabs(["🏆 Top Locations", "📋 Full Table View"])
                ...
                with tab1:
                    ...
                    for _, row in summary.iterrows():
                        loc = row['Location']
                        with st.expander(f"📍 {loc}   |   📥 {row['REQUEST']} Requests   |   📤 {row['REDUCE']} Reduces"):
